...
class orReader:
    ...
    def read(self, categories=[]):
        ...
        for line in f:
            ...
            value = self._entities(self._trimLine(line))
...
if __name__ == "__main__":
    ...
    reader.read([])
    exportPath = os.path.join("D:", "xampp", "htdocs")
    title = "Ranglisten"
    if len(sys.argv) > 1:
        exportPath = sys.argv[1]
        if len(sys.argv) > 2:
            title = sys.argv[2]
        print 'exporting to cmd path {}'.format(sys.argv[1])
    e = HtmlExport(reader.data, exportPath, title)
    e.exportRanking("")
    #e.exportWinnerList("")
